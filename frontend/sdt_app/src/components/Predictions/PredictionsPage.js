...
function generateMockPredictions(days, labels) {
  return days.map((day) => {
    const probs = Array(labels.length)
      .fill(0)
      .map(() => Math.random());
...
const PredictionsPage = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (FORCE_DUMMY) {
        const rows = generateMockPredictions(
          days,
          labels.length ? labels : DEFAULT_LABELS
        );
        if (!cancelled) {
          setPredictions(rows);
          setGroundTruth(null);
      ...
      try {
        ...
        const resp = await apiService.getPredictionsWindow({
          studentId: sid,
          startDay,
          endDay,
          labels,
        ...
        if (!cancelled) {
          ...
          const allErrored =
            ...
            rows.every(
              (r) =>
                typeof r?.pred_label === "string" &&
                r.pred_label.startsWith("ERROR")
            );
          if (mode === "python" && allErrored) {
            rows = generateMockPredictions(
              days,
              labels.length ? labels : DEFAULT_LABELS
            );
            setBackendMode("fallback-data");
          ...
          if (rows.length === days.length)
            rows = rows.map((r, i) => ({ ...r, day: days[i] }));
          setPredictions(rows);
          setGroundTruth(resp && resp.meta ? resp.meta.groundTruth : null);
          if (resp && resp.meta && resp.meta.modelDir)
            setModelDir(resp.meta.modelDir);
        }
      } catch (e) {
        if (!cancelled) {
          setPredictions(
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            ...
            <div className="flex items-center space-x-2 w-full sm:w-auto">
              ...
              <button
                ...
                onClick={() =>
                  setStartDay((d) =>
                    Math.min(Math.max(1, maxDayInData - 4), d + 5)
                  )
                }
              >
                Next 5 ➡️
        ...
        <div className="mt-4 sm:mt-6 bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-200 sdt-dark">
          <div className="mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            <h3 className="text-base sm:text-xl font-semibold text-gray-100">
              Student {selectedStudent} — Days {startDay}–{endDay}
            </h3>
                ...
                <span
                  className={`px-3 py-1 rounded-full text-xs font-medium border ${
                    backendMode === "python"
                      ...
                      : "bg-yellow-200/10 text-yellow-300 border-yellow-400/20"
                  }`}
                >
                  {backendMode === "python"
                    ? "Source: Real models"
