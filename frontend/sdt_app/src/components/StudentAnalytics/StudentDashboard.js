...
const StudentDashboard = ({ currentUser = null }) => {
  ...
  const { theme, isKSG, isMiniDisc } = useTheme();

  // Memoized theme styles
  const cardStyles = useMemo(() => getCardStyles(isKSG), [isKSG]);
  const textStyles = useMemo(() => getTextStyles(isKSG), [isKSG]);
  const chartStyles = useMemo(() => getChartStyles(isKSG), [isKSG]);
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center relative overflow-hidden ${isKSG ? 'bg-ksg-gradient' : 'bg-md-gradient'}`}>
        ...
        <div className="text-center relative z-10">
          <p className={isKSG ? 'text-ksg-lilac' : 'text-md-charcoal'}>No dashboard data available</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`min-h-screen p-6 relative overflow-hidden ${isKSG ? 'bg-ksg-gradient' : 'bg-md-gradient'}`}>
      ...
      <div className="grid grid-cols-1 gap-6 mb-6 relative z-10">
        <div className={`${cardStyles.container} p-1 group transition-all duration-500`}>
          <div className={`${isKSG ? 'rounded-3xl' : 'rounded-xl'} p-8 ${cardStyles.innerGradient}`}>
            <h3
          ...
          <div className="space-y-6">
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <span className={`text-xl font-bold ${isKSG ? 'text-ksg-magenta' : 'text-md-lavender-neon'}`}>
                  {data.currentStats.weeklyEngagement}%
                </span>
              </div>
              <div className={`w-full rounded-full h-4 overflow-hidden ${getProgressBarStyles(isKSG, 0).bg} ${!isKSG ? 'md-progress-bar' : ''}`}>
                <div
                  className={`h-4 rounded-full transition-all duration-700 ease-out ${getProgressBarStyles(isKSG, 0).fill} ${!isKSG ? 'md-progress-fill' : ''}`}
                  style={{
                    width: `${data.currentStats.weeklyEngagement}%`,
                    filter: isKSG ? 'drop-shadow(0 0 4px rgba(255, 156, 238, 0.6))' : 'none'
                  }}
                ></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <span className={`text-xl font-bold ${isKSG ? 'text-ksg-sky' : 'text-md-lavender'}`}>
                  {Math.round(data.currentStats.networkCentrality * 100)}%
                </span>
              </div>
              <div className={`w-full rounded-full h-4 overflow-hidden ${getProgressBarStyles(isKSG, 1).bg} ${!isKSG ? 'md-progress-bar' : ''}`}>
                <div
                  className={`h-4 rounded-full transition-all duration-700 ease-out ${getProgressBarStyles(isKSG, 1).fill} ${!isKSG ? 'md-progress-fill' : ''}`}
                  style={{
                    width: `${data.currentStats.networkCentrality * 100}%`,
                    filter: isKSG ? 'drop-shadow(0 0 4px rgba(160, 196, 255, 0.6))' : 'none'
                  }}
                ></div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6 relative z-10">
        <StatCard
          ...
          index={0}
          rotateClass={isKSG ? "hover:-rotate-1" : ""}
        />
        <StatCard
          icon={Clock}
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={engagementTrendData}>
                ...
                <XAxis
                  ...
                  style={{fontSize: '12px', fontWeight: isKSG ? '500' : '300'}}
                />
                <YAxis
                  stroke={chartStyles.axis}
                  style={{fontSize: '12px', fontWeight: isKSG ? '500' : '300'}}
                ...
                <Line
                  type="monotone"
                  dataKey="engagement"
                  stroke={isKSG ? "#FF9CEE" : "#C4A5F3"}
                  name="Engagement Score"
                  strokeWidth={isKSG ? 3 : 2}
                  dot={{ fill: isKSG ? '#FF9CEE' : '#C4A5F3', strokeWidth: 2, r: 5 }}
                  activeDot={{ r: 7, fill: isKSG ? '#FF9CEE' : '#C4A5F3', strokeWidth: 2 }}
                />
              </LineChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-10">
        <div className="group h-full">
          <div className={`${cardStyles.container} p-1 transition-all duration-500 h-full`}>
            <div className={`${isKSG ? 'rounded-3xl' : 'rounded-xl'} p-8 h-full flex flex-col ${cardStyles.innerGradient}`}>
              <div className="flex items-center gap-2 mb-8">
                <h3
                  className={`text-3xl font-bold ${textStyles.heading}`}
                  style={{
                    ...
                    >
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center font-bold shadow-lg bg-gradient-to-br ${colors.icon} ${
                          isKSG ? 'text-white' : 'text-md-charcoal'
                        }`}
                        style={{
                          filter: isKSG ? 'drop-shadow(0 2px 6px rgba(255, 156, 238, 0.3))' : 'none'
                        }}
                      >
        ...
        <div className="group h-full">
          <div className={`${cardStyles.container} p-1 transition-all duration-500 h-full`}>
            <div className={`${isKSG ? 'rounded-3xl' : 'rounded-xl'} p-8 h-full flex flex-col ${cardStyles.innerGradient}`}>
              ...
              <div className={`flex-1 ${!isKSG ? 'p-4 rounded-lg' : ''}`} style={!isKSG ? {backgroundColor: chartStyles.chartBg} : {}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklySessionData}>
                    <defs>
                      <linearGradient id="barGradient" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={isKSG ? "#FFC6A8" : "#B58AFF"} />
                        <stop offset="80%" stopColor={isKSG ? "#FFD6A5" : "#EAD9FF"} />
                        <stop offset="100%" stopColor={isKSG ? "#FFD6A5" : "rgba(255,255,255,0.6)"} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke={chartStyles.grid} />
                    <XAxis
